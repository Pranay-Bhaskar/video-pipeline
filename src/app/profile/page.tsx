...
export default function ProfilePage() {
  ...
  if (loading || !user) {
    return (
      <div className="h-dvh flex items-center justify-center bg-[#000000]">
  ...
  return (
    <div className="min-h-dvh bg-[#000000] pb-24">
      <div className="bg-gradient-to-b from-[#1a1a1a] to-[#000000] px-4 pt-14 pb-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <div className="w-16 h-16 rounded-2xl bg-white flex items-center justify-center text-xl font-bold text-black border-2 border-[#2a2a2a]">
              {user.fullName.charAt(0).toUpperCase()}
            </div>
            <div>
              <h1 className="text-lg font-bold text-white leading-tight">{user.fullName}</h1>
              <p className="text-xs text-[#a1a1a1] mt-0.5">{user.email}</p>
              <div className="flex items-center gap-1.5 mt-1.5">
                <RoleBadge role={user.role} />
                {user.district && (
                  <span className="flex items-center gap-1 text-[10px] text-[#a1a1a1] bg-[#1a1a1a] px-2 py-0.5 rounded-full">
                    <MapPin className="w-2.5 h-2.5" />
                    {user.district}
                  </span>
                )}
          ...
          <button
            onClick={logout}
            className="w-9 h-9 rounded-xl bg-[#1a1a1a] flex items-center justify-center"
          >
            <LogOut className="w-4 h-4 text-[#a1a1a1]" />
          </button>
        </div>
      ...
      {user.role === "CREATOR" && (
        <div className="px-4">
          <div className="flex bg-[#111111] rounded-2xl p-1 gap-1 mb-4">
            {(["all", "APPROVED", "PENDING", "REJECTED"] as Tab[]).map((tab) => (
              <button
                key={tab}
                onClick={() => handleTabChange(tab)}
      ...
      {user.role === "EXPLORER" && (
        <div className="px-4 space-y-2">
          <p className="text-xs font-medium text-[#6e6e6e] mb-3 mt-2">MY CONTENT</p>
          <MenuItem icon={Bookmark} label="Saved places" sub="0 saved" onClick={() => {}} />
          <MenuItem icon={Video} label="Saved videos" sub="0 videos" onClick={() => {}} />


          <p className="text-xs font-medium text-[#6e6e6e] mb-3 mt-5">ACCOUNT</p>
          <MenuItem icon={Settings} label="Settings" sub="Preferences & notifications" onClick={() => {}} />
          <MenuItem
            icon={LogOut}
            label="Sign out"
      ...
      {user.role === "CREATOR" && (
        <div className="px-4 mt-6 space-y-2">
          <p className="text-xs font-medium text-[#6e6e6e] mb-3">ACCOUNT</p>
          <MenuItem icon={Settings} label="Settings" sub="Preferences & notifications" onClick={() => {}} />
          <MenuItem
            icon={LogOut}
            label="Sign out"
...
function StatCard({ value, label, color }: { value: number; label: string; color: string }) {
  return (
    <div className="bg-[#111111] rounded-xl p-3 text-center border border-[#2a2a2a]">
      <div className={cn("text-xl font-bold", color)}>{value}</div>
      <div className="text-[10px] text-[#6e6e6e] mt-0.5">{label}</div>
    </div>
  );
}
...
function VideoListItem({ video }: { video: IVideo }) {
  ...
  return (
    <div className="bg-[#111111] rounded-2xl border border-[#2a2a2a] overflow-hidden flex">
      <div className="w-20 h-20 bg-[#1a1a1a] flex-shrink-0 relative">
        {video.thumbnailUrl ? (
          <img src={video.thumbnailUrl} alt="" className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            <Video className="w-6 h-6 text-[#2a2a2a]" />
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent" />
      </div>
      ...
      <div className="flex-1 p-3 min-w-0">
        ...
        {category && (
          <span className="text-[10px] text-[#6e6e6e] mt-0.5 block">{category.emoji} {category.label}</span>
        )}
        <div className="flex items-center justify-between mt-2">
          <div className={cn("flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full", status.bg, status.color)}>
            <StatusIcon className="w-2.5 h-2.5" />
...
function MenuItem({
  ...
  return (
    <button
      onClick={onClick}
      className="w-full bg-[#111111] rounded-2xl border border-[#2a2a2a] px-4 py-3.5 flex items-center gap-3 active:opacity-70 transition-all"
    >
      <div className={cn("w-9 h-9 rounded-xl flex items-center justify-center",
        danger ? "bg-rose-500/10" : "bg-[#1a1a1a]"
      )}>
        <Icon className={cn("w-4.5 h-4.5", danger ? "text-rose-400" : "text-white")} style={{ width: 18, height: 18 }} />
      </div>
      <div className="flex-1 text-left">
        <p className={cn("text-sm font-medium", danger ? "text-rose-400" : "text-white")}>{label}</p>
        <p className="text-xs text-[#6e6e6e] mt-0.5">{sub}</p>
      </div>
      {!danger && <ChevronRight className="w-4 h-4 text-[#2a2a2a]" />}
    </button>
  );
}
...
function EmptyVideos({ tab }: { tab: Tab }) {
  ...
  return (
    <div className="py-16 flex flex-col items-center text-center">
      <div className="text-4xl mb-3">{m.emoji}</div>
      <p className="text-sm font-semibold text-white">{m.title}</p>
      <p className="text-xs text-[#6e6e6e] mt-1">{m.sub}</p>
    </div>
  );
}
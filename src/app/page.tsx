...
export default function HomePage() {
  ...
  if (loading) {
    return (
      <div className="h-dvh flex flex-col items-center justify-center bg-[#000000]">
        <div className="w-14 h-14 rounded-2xl bg-[#000000] border border-[#2a2a2a] flex items-center justify-center mb-4">
          <MapPin className="w-7 h-7 text-white" />
        </div>
        <p className="text-sm text-[#a1a1a1]">Loading stories...</p>
        <Loader2 className="w-5 h-5 text-white animate-spin mt-3" />
      </div>
    );
  }
  ...
  if (videos.length === 0) {
    return (
      <div className="h-dvh flex flex-col items-center justify-center bg-[#000000] px-8 text-center">
        <div className="text-5xl mb-4">🌿</div>
        <h2 className="text-lg font-bold text-white mb-2">No stories yet</h2>
        <p className="text-sm text-[#a1a1a1]">Be the first to share an incredible Karnataka story</p>
        <BottomNav />
      </div>
    );
  }
  ...
  return (
    <div className="relative h-dvh bg-black">
      <div ref={containerRef} className="h-dvh overflow-y-scroll" style={{ scrollSnapType: "y mandatory" }}>
        ...
        {!hasMore && (
          <div className="h-dvh flex items-center justify-center bg-[#000000]" style={{ scrollSnapAlign: "start" }}>
            <div className="text-center">
              <div className="text-4xl mb-3">🎬</div>
              <p className="text-white font-semibold">You have seen it all</p>
              <p className="text-sm text-[#a1a1a1] mt-1">Check back for new stories</p>
            </div>
          </div>
        )}
      </div>

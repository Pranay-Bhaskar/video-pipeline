...
export default function UploadPage() {
  ...
  if (step === "pick") {
    return (
      <div className="min-h-dvh bg-[#000000] flex flex-col">
        <div className="flex items-center gap-3 px-4 pt-14 pb-4">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 pb-24">
          <div
            ...
            className={cn(
              "w-full aspect-[9/16] max-h-[55dvh] rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-4 transition-all cursor-pointer",
              dragOver ? "border-white bg-[#1a1a1a]" : "border-[#2a2a2a] bg-[#111111] hover:border-[#6e6e6e]/50"
            )}
          >
            ...
            <div className="text-center">
              <p className="text-white font-semibold">Tap to upload video</p>
              <p className="text-sm text-[#a1a1a1] mt-1">MP4, MOV, WEBM · max 200MB</p>
              <p className="text-xs text-[#a1a1a1] mt-0.5">Vertical videos work best</p>
            </div>
            <div className="bg-white rounded-full px-5 py-2">
              <span className="text-sm font-semibold text-black">Choose file</span>
            </div>
          </div>
  ...
  if (step === "done") {
    return (
      <div className="min-h-dvh bg-[#000000] flex flex-col items-center justify-center px-6 text-center">
        <div className="w-20 h-20 rounded-full bg-emerald-500/15 flex items-center justify-center mb-5">
          <CheckCircle className="w-10 h-10 text-emerald-400" />
        </div>
        <h2 className="text-xl font-bold text-white mb-2">Uploaded!</h2>
        <p className="text-sm text-[#a1a1a1] mb-2">Your story is under admin review.</p>
        <p className="text-xs text-[#a1a1a1] mb-8">It will appear in the feed once approved.</p>
        <button
          onClick={() => router.push("/profile")}
          className="bg-white text-black font-semibold px-8 py-3 rounded-xl mr-3"
        >
          View my videos
        </button>
        <button
          onClick={() => { setStep("pick"); setFile(null); setPreview(null); setProgress(0); setForm({ title:"",description:"",category:"",placeName:"",district:"",latitude:"",longitude:"",tags:"" }); }}
          className="text-[#a1a1a1] font-medium mt-4 block"
        >
          Upload another
        </button>
      </div>
  ...
  if (step === "uploading") {
    return (
      <div className="min-h-dvh bg-[#000000] flex flex-col items-center justify-center px-8 text-center">
        <div className="w-20 h-20 rounded-full bg-[#1a1a1a] flex items-center justify-center mb-5">
          <Upload className="w-10 h-10 text-white" />
        </div>
        <h2 className="text-xl font-bold text-white mb-2">Uploading...</h2>
        <p className="text-sm text-[#a1a1a1] mb-6">{progress}% complete</p>
        ...
        <p className="text-xs text-[#a1a1a1] mt-4">Don&apos;t close this tab</p>
      </div>
    );
  }
  ...
  // ── STEP: Details form ──
  return (
    <div className="min-h-dvh bg-[#000000] flex flex-col overflow-y-auto">
      <div className="flex items-center gap-3 px-4 pt-14 pb-3 border-b border-[#1a1a1a]">
        <button onClick={() => { setStep("pick"); setFile(null); setPreview(null); }} className="w-9 h-9 rounded-full bg-[#1a1a1a] flex items-center justify-center">
          <ArrowLeft className="w-4 h-4 text-white" />
        </button>
        <h1 className="text-lg font-bold text-white flex-1">Story details</h1>
      </div>
      ...
      <div className="flex gap-3 items-start px-4 py-4 border-b border-[#1a1a1a]">
        {preview && (
          <div className="relative w-16 h-24 rounded-xl overflow-hidden flex-shrink-0 bg-[#1a1a1a]">
            <video src={preview} className="w-full h-full object-cover" muted />
            <button
              onClick={() => { setStep("pick"); setFile(null); setPreview(null); }}
              className="absolute top-1 right-1 w-5 h-5 rounded-full bg-black/60 flex items-center justify-center"
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-white truncate">{file?.name}</p>
          <p className="text-xs text-[#a1a1a1] mt-1">{file ? `${(file.size / 1024 / 1024).toFixed(1)} MB` : ""}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="px-4 py-4 space-y-4 pb-28">
        ...
        <div className="grid grid-cols-2 gap-3">
          <FormField label="Place name *">
            <div className="relative">
              <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#6e6e6e]" />
              <input
                value={form.placeName}
                onChange={(e) => setForm({ ...form, placeName: e.target.value })}
                placeholder="Jog Falls"
        ...
        <FormField label="Tags (comma separated)">
          <div className="relative">
            <Tag className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#6e6e6e]" />
            <input
              value={form.tags}
              onChange={(e) => setForm({ ...form, tags: e.target.value })}
              placeholder="monsoon, waterfall, hidden gem"
        ...
        <button
          type="submit"
          className="w-full bg-white hover:bg-[#f5f5f5] active:scale-[0.98] text-black font-semibold py-4 rounded-xl transition-all flex items-center justify-center gap-2"
        >
          <Upload className="w-5 h-5" />
          Submit for review
        </button>
...
function FormField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="text-xs font-medium text-[#a1a1a1] mb-1.5 block">{label}</label>
      {children}
    </div>
  );
}
...
const inputCls =
  "w-full bg-[#111111] border border-[#2a2a2a] rounded-xl px-4 py-3 text-sm text-white placeholder-[#6e6e6e] focus:border-white focus:ring-1 focus:ring-white transition-all";
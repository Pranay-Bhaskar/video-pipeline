...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-[#000000]/95 backdrop-blur-md border-t border-[#2a2a2a] z-50">
      <div className="flex items-center justify-around px-2 py-2 pb-safe">
        {visibleItems.map(({ href, icon: Icon, label }) => {
          const isActive = pathname === href;
          const isUpload = href === "/upload";
          return (
            ...
            >
              {isUpload ? (
                <div className="w-12 h-12 rounded-2xl bg-[#000000] border border-[#2a2a2a] flex items-center justify-center shadow-lg shadow-black/40 -mt-5">
                  <Icon className="w-5 h-5 text-white" />
                </div>
              ) : (
                <>
                  <Icon
                    className={cn(
                      "w-6 h-6 transition-all",
                      isActive ? "text-white" : "text-[#6e6e6e]"
                    )}
                  />
                  <span
                    className={cn(
